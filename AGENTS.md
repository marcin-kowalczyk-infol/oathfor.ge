...
- Available now: `python3 .agents/commands/check_repository.py` (Python 3.11+), `git diff --check`. Full isolated API/service check: `python3 .agents/commands/check_api.py` (Docker required). API checks from `apps/api`: `composer test`, `composer test:integration`, `composer analyse`. Mobile from `apps/mobile`: `npm test -- --runInBand`, `npm run typecheck` (Node from `.nvmrc`). Setup/configuration and local server commands: [development guide](docs/engineering/development.md). **Source: [command catalog](.agents/commands/README.md).**
